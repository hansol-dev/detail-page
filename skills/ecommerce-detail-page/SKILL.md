...
- After all cut images are complete in the web tool, show them in the image review page with per-cut download, full-detail preview, and revision controls. Do not require a separate HTML gallery file.
...
## Ddookso Mandatory Last Cut

For this user's ecommerce detail-page image production workflow, always include the fixed seller notice image as the final cut.

- Fixed asset path: `assets/ddookso-policy-last-cut.png`.
- The final image in every generated detail-page set must be this asset, unchanged, unless the user explicitly asks to replace or remove it.
- Count this asset inside the selected cut count. Example: for a 12-cut page, create 11 product/detail cuts and use `assets/ddookso-policy-last-cut.png` as cut 12.
- Do not regenerate, rewrite, translate, crop, or restyle this final notice image by default.
- If an HTML gallery or download page is built, append this asset as the last item in numerical order.
