  ...
  return {
    ...
    cuts: Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        photoPlacement: isNoticeCut ? "상품 사진보다 안내 정보 카드와 아이콘 중심으로 구성" : "제공된 상품 사진은 제품 형태와 라벨 확인용 레퍼런스로 참고하고, 상세페이지 컷에 맞게 새로 연출",
    ...
    const baseFallback =
      !hasNoticeText && index === noticeIndex
        ? {
            ...
            photoPlacement: "상품 사진은 제품 형태와 라벨 확인용 레퍼런스로 참고하고, 상세페이지 컷에 맞게 새로 연출한 제품 비주얼을 중심에 배치",
      ...
      messages: [
        {
          ...
          content: [
            ...
            "If uploaded product photos exist, treat them as the product appearance source of truth for identity, package shape, label, color, contents, and options, but plan them as visual references. Do not instruct the image model to paste raw uploaded photos unchanged; each cut should look newly designed for an ecommerce detail page.",
